...
template <typename AIE, int X, int Y> struct prog : acap::aie::tile<AIE, X, Y> {
  ...
  bool prerun() {
    ...
    t::get_dev_handle().mem_write(acap::hw::offset_table::get_tile_mem_begin_offset(), written_data.back().x);
    /// writes to tile_memory::y
    t::get_dev_handle().mem_write(acap::hw::offset_table::get_tile_mem_begin_offset() + 4, written_data.back().y);
    /// writes to tile_memory::id
    t::get_dev_handle().mem_write(acap::hw::offset_table::get_tile_mem_begin_offset() + 8, written_data.back().id);
  ...
  void postrun() {
    t::get_dev_handle().memcpy_d2h(
        &output, acap::hw::offset_table::get_tile_mem_begin_offset() + 12,
        sizeof(output));
