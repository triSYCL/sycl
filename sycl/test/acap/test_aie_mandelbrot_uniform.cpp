...
// notable issues:
// 
//  - if graphics::application is a local variable it needs to be captured by the lambda.
// kernel lambda need to capture by value ([=]) but this causes a copy. and on the host
// graphics::application contain std::thread which aren't copyable.
// This was fixed by making graphics::application a global variable.
// 
//  - constness of the lambda
//  - kernel_outlining
