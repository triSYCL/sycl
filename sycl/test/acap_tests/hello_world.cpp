/*
 No device compiler command, just cpu/host x86, no cross compilation:
  ...
  Cross compilation for ARM host CPU/PS with AI Engine device compilation:
     ...
     -aux-triple should be equal to the triple as without it there is a
     disconnect in the CompilerInstance/CompilerInvocation where it will use the
     triple of the system that's compiling the binary. For example if you invoke
     this without the aux-triple on an x86 you'll end up with some x86 compiler
     definitions and builtins as the InitPreprocessor will ignore the -target
     and prioritize the aux-triple, which when not defined defaults to the
     compiling system
