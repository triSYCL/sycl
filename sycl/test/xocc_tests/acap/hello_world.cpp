/*
 No device compiler command, just cpu/host x86, no cross compilation:
  clang++ -std=c++2a hello_world.cpp -I/storage/ogozillo/ACAP++/acappp/include \
  ...
  // This makes life much simpler than it could be.. the rest is normal ACAP++ includes
  $ISYCL_BIN_DIR/clang++ -std=c++2a -target aarch64-linux-gnu -mcpu=cortex-a72 \
    ...
    `pkg-config gtkmm-3.0 --cflags` `pkg-config gtkmm-3.0 --libs` \
    -I/storage/ogozillo/ACAP++/acappp/include  hello_world.cpp
