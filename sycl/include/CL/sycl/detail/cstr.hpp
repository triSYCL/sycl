...
#else
  __SYCL_ALWAYS_INLINE constexpr cstr<char> operator"" _cstr(const char *,
                                                             size_t) noexcept {
    return {};
  }

  /// When not in device mode the kernel properties are not used so they do not
  /// need to be correct just to compile correctly.
  using Base2 = cstr<char, '0'>;
  using Base10 = cstr<char, '0'>;
  using Base16 = cstr<char, '0'>;
