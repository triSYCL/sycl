...
_BitInt(33) Declarations(_BitInt(48) &Param) { // Useable in params and returns.
  ...
  _BitInt(-3) g; // expected-error{{signed _BitInt must have a bit size of at least 1}}
  ...
  signed _BitInt(0) m; // expected-error{{signed _BitInt must have a bit size of at least 1}}
