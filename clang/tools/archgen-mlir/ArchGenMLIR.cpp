...
// doesn't support the plugin behavior I needed so I slightly modified clang to
// get the behavior i needed.
...
//  - our plugin will split AST component based on whether it needs to handled
//  them itself or need to hand them of the stand IR-gen
//  - The parts of the AST that are generated by the plugin it self are very
...
//  what it needs to. To find everything that need to get generated, we do a BFS
//  traversal of relevant part of the the functions call graph.
//  - All the function that need generation will be generated to MILR, in a mix
...
//  transform the abstract description of what computation to perform into a
//  concrete way to approximate the requested computation. The output of
//  LowerApprox is mostly some FixedPt operation.
...
//  - after that ArchGenMLIR has do it job, the approximation function is into
...
public:
  ...
  mlir::LogicalResult runMLIROptimizationAndLowering() {
...

    {
      ...
      /// form here then from the eval call in the .cpp
  ...
  /// Kinda of the main function for our plugin the only thing happening befor
  /// this is the ASTConsumer making the list of every top-level function we
  /// need to generate.
  mlir::LogicalResult Finalize() {
    ...
    /// generating MLIR for an function.
