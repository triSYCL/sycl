...
//===----------------------------------------------------------------------===//
//
// Hybrid (LLVMIR, MLIR) IR-gen used for the compiler implementation based on
// archgenlib.
// The Compiler part of ArchGenMLIR is contained inside a clang plugin. clang
// doesn't support the plugin behavior needed so clang has been slightly modified to
// get the needed behavior.
// Here is an overview of the compilation part of ArchGenMLIR:
//  - standard clang lexer + parser + sema
//  - our plugin will be the unique ASTConsumer of the clang frontend and have
//  access to the standard clang IR-gen
//  - our plugin will split AST components based on whether it needs to handle
//  them itself or needs to hand them to the standard IR-gen;
//  - The parts of the AST that are generated by the plugin itself are very
//  well controlled because they come from the Marto library
//  - If the plugin handles an AST component itself, it will at first just add
//  them to the list of stuff to generate
//  - Then during the Finalizer/HandleTranslationUnit, it will generate MLIR
//  that it needs to. To find everything that need to get generated, we do a BFS
//  traversal of relevant part of the the functions call graph.
//  - All the functions that need generation will be generated in MLIR, in a mix
//  of Func, Approx and LLVM dialects
//  - That mix will be inlined and preprocessed. into a canonical
//  representation of mathematical functions.
//  - we do the core of ArchGenMLIR, LowerApprox. LowerApprox will
//  transform the abstract description of which computation to perform into a
//  concrete way to approximate the requested computation. The output of
//  LowerApprox is mostly some FixedPt operations.
//  - We progressively lower and optimize FixedPt -> Arith -> LLVM until
//  everything is only LLVM Dialect in MLIR
//  - We translate LLVM Dialect to LLVMIR and Link it into the LLVM Module used
//  by standard clang IR-gen.
//  - after that ArchGenMLIR has done its job, the approximation function is into
//  the LLVMIR and the pipeline can continue normally
//
//===----------------------------------------------------------------------===//
...
class Util {
  ...
  static bool matchContext(const clang::DeclContext *Ctx,
    ...
    // Check the declaration context chain starting from the item
...
public:
  /// Top-level function that should be used. Takes a type and the name of the
  /// archgen class it is expected to represent. it will return true if the type
  /// is of that class.
...
public:
  ...
  const clang::TemplateArgumentList &getTemplateArgList(clang::QualType Ty) {
    return cast<clang::ClassTemplateSpecializationDecl>(Ty->getAsRecordDecl())
        ->getTemplateArgs();
  }
  ...
  mlir::Type getMLIRType(clang::QualType Cqtype) {
    ...
    /// because they are composites of other potentially complex types and we
    /// dont want to generate types more then once.
  ...
  func::FuncOp GetOrCreateFunc(clang::FunctionDecl *FD) {
    ...
    /// If it is not in the map, it is not going to get emitted on its own.
    /// So we add it to the list of functions to emit
    FuncToEmit.push_back(FD);

    /// The top-level function need to stay, all the rest should be deleted by
    /// inlining
    ...
    /// being generated yet
    /// The body will be filled later and the parameter will be fixed after
    ...
    MLIRFunc.setSymVisibilityAttr(visibility);
    return MLIRFunc;
  }
...
struct MLIREmitter : public clang::StmtVisitor<MLIREmitter, mlir::Value> {
  ...
  mlir::Value VisitIntegerLiteral(clang::IntegerLiteral *IL) {
    ...
    return state.builder
        .create<arith::ConstantIntOp>(state.getMLIRLocation(IL->getLocation()),
  ...
  mlir::Value VisitGenericOpCall(clang::CallExpr *CE) {
    ...
    if (Kind == Annot::KindConstant) {
      ...
      auto attr = fixedpt::FixedPointAttr::get(&state.ctx, std::move(FPvalue));
      return state.builder
...
/// List all Types and Statements that need to be generated for this to work
...
public:
  ...
  mlir::LogicalResult runMLIROptimizationAndLowering() {
    ...
    /// At this point the front-end and cleanup is done
...

    {
      ...
      /// When debugging it is a lot more useful to see the source locations
      /// from here than from the evaluation call in the .cpp
  ...
  /// This is equivalent to the main function for our plugin the only thing
  /// happening before this is the ASTConsumer making the list of every
  /// top-level function we need to generate.
  mlir::LogicalResult Finalize() {
    ...
    /// This is a BFS transversal of the call graph where finding edges means
    /// generating MLIR for a function.
  ...
  // Below is the exposed API of clang::ASTConsumer, most functions just need to
  ...
  bool HandleTopLevelDecl(clang::DeclGroupRef D) override {
    if (llvm::any_of(D, [&](auto *E) {
      ...
      /// We assume here that the DeclGroupRef contains only code we
...
class ArchGenMLIRAction : public clang::PluginASTAction {
  ...
  /// Original Main actions being replaced. It should be usually a
...
public:
  ...
  /// clang::FrontendAction. So, before, this function needs to copy our data to
  /// the Inner one
